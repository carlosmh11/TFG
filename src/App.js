...
const DashboardDatosMeteorológicos = () => {
  ...
  useEffect(() => {
      axios.get('https://s77wrglsok.execute-api.eu-central-1.amazonaws.com/dev/datos-pronostico')
      .then(response => {
  ...
  useEffect(() => {
    if (data && mesSeleccionado && filtroParajes) {
      ...
      // Convertir las fechas a objetos Date usando moment
      filteredData.forEach(registro => {
        registro['FECHA_OBJ'] = moment(registro['FECHA'], 'DD/MM/YYYY').toDate(); // Guardar la fecha original
      });

      // Ordenar los datos por fecha
      filteredData.sort((a, b) => a['FECHA_OBJ'] - b['FECHA_OBJ']);

      // Formatear las fechas nuevamente al formato original 'DD/MM/YYYY'
      const fechas = filteredData.map(registro => moment(registro['FECHA_OBJ']).format('DD/MM/YYYY'));
